...
export default function TriggerProofPanel() {
  ...
  async function loadAll() {
    ...
    try {
      const [platform, oracle] = await Promise.all([
        fetch(`${API}/admin/panel/proof/platform-activity`).then(r => r.json()),
        fetch(`${API}/admin/panel/proof/oracle-reliability`).then(r => r.json()),
      ]);
  ...
  async function loadPartnerActivity() {
    ...
    try {
      const res = await fetch(`${API}/zones/partners/${partnerId}/activity`);
      if (!res.ok) throw new Error(`Partner ${partnerId} not found`);
      setActivityForm(await res.json());
      const eligRes = await fetch(`${API}/zones/partners/${partnerId}/activity/eligibility`);
