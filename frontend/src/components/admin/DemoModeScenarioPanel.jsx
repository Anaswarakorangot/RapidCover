...
export default function DemoModeScenarioPanel() {
  ...
  async function loadAll() {
    ...
    try {
      const [statusRes, scenariosRes] = await Promise.all([
        adminFetch(`${API}/admin/panel/demo-mode/status`),
        adminFetch(`${API}/admin/panel/demo-mode/scenarios`),
      ]);
  ...
  async function handleToggle() {
    ...
    setToggling(true);
    try {
      const res = await adminFetch(`${API}/admin/panel/demo-mode/toggle?enabled=${!demoStatus.enabled}`, { method: 'POST' });
  ...
  async function handleRun(event) {
    ...
    setSubmitting(true);
    try {
      const res = await adminFetch(`${API}/admin/panel/demo-mode/run`, {
        method: 'POST',
  ...
  async function handleSelectRun(runId) {
    try {
      const res = await adminFetch(`${API}/admin/panel/demo-mode/run/${runId}`);
      const data = await res.json();
  ...
  async function handleCleanup(runId) {
    ...
    try {
      const res = await adminFetch(`${API}/admin/panel/demo-mode/run/${runId}/cleanup`, { method: 'POST' });
      const data = await res.json();
