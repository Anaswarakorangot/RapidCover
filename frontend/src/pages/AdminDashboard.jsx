...
// Phase 3: Admin Authentication & Hardened Security
          ...
          <div className="topnav-right">
            ...
            <div className="topnav-profile">
              <div className="profile-info" style={{ textAlign: 'right' }}>
                <div className="profile-name">{admin?.full_name || 'Admin User'}</div>
                <div className="profile-role">Platform Manager</div>
              </div>
              <div className="profile-avatar">{admin?.full_name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'AD'}</div>
            </div>
            <button
              onClick={() => {
                logout();
